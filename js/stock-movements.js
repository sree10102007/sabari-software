  ...
  function renderMovements(movements) {
    ...
    movements.forEach(m => {
      ...
      tr.innerHTML = `
        ...
        <td>
          <button class="btn btn-danger btn-sm btn-delete-sm" data-id="${m.id}" data-name="${escapeHtml(m.material_name)}" data-qty="${m.quantity}" data-type="${m.movement_type}">🗑️ Delete</button>
        </td>
      `;

      tr.querySelector('.btn-delete-sm').addEventListener('click', async (e) => {
        const smId = parseInt(e.target.dataset.id);
        const mName = e.target.dataset.name;
        ...
        try {
          ...
          if (result.success) {
            showToast('🗑️ Stock movement log deleted and stock reversed!');
            loadMovements();
          } else {
