document.addEventListener('DOMContentLoaded', () => {
      ...
      tr.innerHTML = `
        ...
        <td>
          <div class="action-buttons">
            <button class="btn btn-secondary btn-sm btn-edit-mat" data-id="${m.id}">✏️ Edit</button>
            <a href="add-stock.html?material_id=${m.id}" class="btn btn-success btn-sm" style="text-decoration:none;">📥 Add Stock</a>
            <button class="btn btn-danger btn-sm btn-delete-mat" data-id="${m.id}" data-name="${escapeHtml(m.name)}">🗑️ Delete</button>
          </div>
        </td>
      `;
      
      tr.querySelector('.btn-edit-mat').addEventListener('click', (e) => {
        openEditModal(parseInt(e.target.dataset.id));
      });
      
      tr.querySelector('.btn-delete-mat').addEventListener('click', (e) => {
        deleteMaterial(parseInt(e.target.dataset.id), e.target.dataset.name);
      });
  ...
  async function deleteMaterial(id, name) {
    ...
    try {
      ...
      if (result.success) {
        showToast(`🗑️ Material "${name}" deleted.`);
        loadMaterials();
      } else {
