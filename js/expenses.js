...
document.addEventListener('DOMContentLoaded', async () => {
      ...
      tr.innerHTML = `
        ...
        <td>
          <div style="display:flex;gap:6px;">
            <button class="btn btn-secondary btn-sm btn-edit-v" data-id="${item.id}">✏️ Edit</button>
            <button class="btn btn-danger btn-sm btn-delete-v" data-id="${item.id}" data-num="${escapeHtml(item.vehicle_number)}">🗑️ Delete</button>
          </div>
        </td>
      `;
  ...
  function openVehicleEdit(item) {
    ...
    vehicleModal.style.display = 'flex';
    document.getElementById('vehicle-modal-title').textContent = '✏️ Edit Vehicle Expense';
  }
  ...
  function renderPersonalExpenses(items) {
    ...
    items.forEach(item => {
      ...
      tr.innerHTML = `
        ...
        <td>
          <div style="display:flex;gap:6px;">
            <button class="btn btn-secondary btn-sm btn-edit-p" data-id="${item.id}">✏️ Edit</button>
            <button class="btn btn-danger btn-sm btn-delete-p" data-id="${item.id}" data-emp="${escapeHtml(item.employee_name)}">🗑️ Delete</button>
          </div>
        </td>
      `;
  ...
  function openPersonalEdit(item) {
    ...
    personalModal.style.display = 'flex';
    document.getElementById('personal-modal-title').textContent = '✏️ Edit Personal Expense';
  }
