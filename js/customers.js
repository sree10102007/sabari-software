document.addEventListener('DOMContentLoaded', () => {
  ...
  function applyFilters() {
    ...
    const filtered = allCustomers.filter(c => {
      ...
      const matchesType = activeTypeFilter === 'All' || (c.customer_type || 'Direct Customer') === activeTypeFilter;
  ...
  function renderTable(customers) {
    ...
    customers.forEach(c => {
      ...
      tr.innerHTML = `
        ...
        <td><span class="status-badge ${c.customer_type === 'Engineer' ? 'approved' : 'pending'}">${escapeHtml(c.customer_type || 'Direct Customer')}</span></td>
        ...
        <td>
          <div class="action-buttons">
            <button class="btn btn-secondary btn-sm btn-view-history" data-id="${c.id}" data-name="${escapeHtml(c.name)}">📜 History</button>
            <button class="btn btn-primary btn-sm btn-edit-cust" data-id="${c.id}">✏️ Edit</button>
            <a href="stock-out.html" class="btn btn-success btn-sm" style="text-decoration:none;">📤 Sale</a>
            <button class="btn btn-danger btn-sm btn-delete-cust" data-id="${c.id}" data-name="${escapeHtml(c.name)}">🗑️ Delete</button>
          </div>
        </td>
      `;
      
      tr.querySelector('.btn-view-history').addEventListener('click', (e) => {
        viewHistory(parseInt(e.target.dataset.id), e.target.dataset.name);
      });
      
      tr.querySelector('.btn-edit-cust').addEventListener('click', (e) => {
        openEditCust(parseInt(e.target.dataset.id));
      });
      
      tr.querySelector('.btn-delete-cust').addEventListener('click', (e) => {
        deleteCustomer(parseInt(e.target.dataset.id), e.target.dataset.name);
      });
  ...
  function openEditCust(id) {
    ...
    document.getElementById('edit-cust-type').value = c.customer_type || 'Direct Customer';
  ...
  async function deleteCustomer(id, name) {
    ...
    try {
      ...
      if (result.success) {
        showToast(`🗑️ Customer "${name}" deleted successfully.`);
        loadCustomers();
      } else {
