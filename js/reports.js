...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  function initReports() {
    ...
    btnExportCsv.addEventListener('click', exportActiveReportToCSV);
  ...
  function exportActiveReportToCSV() {
    if (!reportDataCache) {
      showToast('No report data generated yet to export.', 'error');
      return;
    }

    const activeSec = document.getElementById('report-' + activeReportTab);
    const tables = activeSec.querySelectorAll('table');
    if (!tables.length) {
      showToast('No tables found for active report.', 'error');
      return;
    }
    ...
    tables.forEach((table, index) => {
      // Add table header title if present
      const headerTitle = table.previousElementSibling?.textContent || `Table ${index + 1}`;
      csvContent += `"${headerTitle.trim()}"\n`;

      const rows = table.querySelectorAll('tr');
      rows.forEach(tr => {
        const cols = tr.querySelectorAll('th, td');
        const rowData = Array.from(cols).map(col => {
          // Replace linebreaks and escape double quotes
    ...
    link.setAttribute("download", `sivakami_${activeReportTab}_report_${new Date().toISOString().split('T')[0]}.csv`);
