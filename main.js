...
ipcMain.handle('db:deleteMaterial', async (event, id) => {
  try {
    const hasHistory = await db.queryOne(
      `SELECT 1 FROM stock_movements WHERE material_id = ? AND (is_deleted IS NULL OR is_deleted = 0) LIMIT 1`,
      [id]
    );
    if (hasHistory) {
      return { success: false, error: 'This record is referenced by existing transactions.' };
    }
  ...
  try {
    ...
    try {
      ...
      if (existingCust) {
        ...
        await db.run(
          'UPDATE customers SET total_purchases = total_purchases + ?, balance_amount = balance_amount + ? WHERE id = ?',
          [total_amount || 0, balance_amount || 0, customerId]
        );
      } else {
        const custResult = await db.run(
          `INSERT INTO customers (name, phone, address, total_purchases, balance_amount, created_at)
           VALUES (?, ?, ?, ?, ?, ?)`,
          [customer_name, customer_phone || '', customer_address || '', total_amount || 0, balance_amount || 0, now]
        );
      ...
      const receiptResult = await db.run(
        ...
        [receiptNumber, customerId, customer_name, customer_phone || '', customer_address || '',
          timestamp, total_amount || 0, paid_amount || 0, balance_amount || 0, remarks || '', sale_type || 'Stock Out', now]
      );
      ...
      await db.run(
        `INSERT INTO stock_movements (material_id, movement_type, quantity, customer_id, customer_name, receipt_id, rate, total_amount, paid_amount, balance_amount, remarks, created_by, created_at, stock_direction)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'OUT')`,
        [material_id, sale_type || 'Stock Out', quantity, customerId, customer_name,
    ...
    try {
      ...
      if (dir === 'IN') {
        ...
        if (mat && (mat.current_stock - sm.quantity < 0)) {
          return { success: false, error: `Cannot reverse stock. Reversing this entry would make stock for '${mat.name}' negative.` };
        }
...
ipcMain.handle('db:deleteCustomer', async (event, id) => {
  try {
    const hasHistory = await db.queryOne(
      `SELECT 1 FROM receipts WHERE customer_id = ? AND (is_deleted IS NULL OR is_deleted = 0) LIMIT 1`,
      [id]
    );
    if (hasHistory) {
      return { success: false, error: 'This record is referenced by existing transactions.' };
...
ipcMain.handle('db:addCustomer', async (event, { name, phone, address, customer_type }) => {
  try {
    ...
    const result = await db.run(
      ...
      [name, phone || '', address || '', customer_type || 'Direct Customer', now]
    );
...
ipcMain.handle('db:updateCustomer', async (event, { id, name, phone, address, customer_type }) => {
  try {
    ...
    await db.run('UPDATE customers SET name=?, phone=?, address=?, customer_type=? WHERE id=?', [name, phone || '', address || '', customer_type || 'Direct Customer', id]);
    return { success: true };
  } catch (err) { return { success: false, error: err.message }; }
});
